...
> Stand: Addon-Version **0.9.36** (**MVP-Raidframes-Block KOMPLETT** — alle §7-Features umgesetzt & live bestätigt; jetzt im Aufbau: **Suite-Shell**). **Suite-Shell Phase 1 (v0.9.31, gemergt) + Phase 2 Widget-Toolkit (v0.9.32–.36, live bestätigt im Screenshot-Loop):** eigene gerunte Config-Optik (`Shell/`), läuft PARALLEL zur AceConfig (`/lumen shell` öffnet sie, `/lumen` bleibt klassisch). Phase 1 = Chrome (Header/Nav/Tabs/Footer/Rune-Ecken), Tokens, Fonts (Cinzel + Hanken Grotesk). Phase 2 = `ns.W`-Widget-Toolkit (Slider/Select/Checkbox/Button/Card/GroupPanel/IconTile/Row) auf den Tokens; ALLE Maße zentral in `UI.WIDGET`, Fonts in `UI.ROLE` (Florian soll selbst feinjustieren). RenderDummy = live bedienbare Widget-Galerie. Details §10.11 + Memory `lumen-suite-shell-preview-plan`. **Davor (MVP-Raidframes):** v0.9.21–.25 Folge-Fixes nach der v0.9.20-Regression (HP-%/Auren-Crash via `UnitHealthPercent`+`ScaleTo100`; **Talent→Aura-Remap** `TALENT_TO_AURA`/`ResolveTrackId`, Germination 155675→155777; Click-Cast sichere Defaults; Heiler-IDs inkl. Dawnlight 431381). **v0.9.28 Aggro** (§10.9) + **v0.9.30 Sortierung Rolle/Gruppe** (§10.10), live bestätigt. **Offen: Debuff-Live-Test** (Florian, braucht Gruppe). **Temp `/ldump`-Debugkommando in `Core.lua` — vor Release entfernen.** **Nächster Schritt: Suite-Shell Phase 3** (erster echter Screen Raidframes→Raid ans `db.profile` verdrahtet; danach restliche Screens + angedockte Live-Vorschau, Testmodus geht darin auf). Interface **120007** (Retail-Patch 12.0.7, live seit 16.06.2026). Tiefe Details: Memories `lumen-aura-phase2-progress`, `lumen-suite-shell-preview-plan`.
...
|---|---|
| `Lumen.toc` | Deklariert Addon. `## Interface: 120007`, `## SavedVariables: LumenDB`, `## Author: NennMichSchinken`, `## Version: 0.9.36`. Lädt in Reihenfolge: `embeds.xml`, `Core.lua`, `EditMode.lua`, `Style.lua`, `Shell\Tokens.lua`, `Shell\Widgets.lua`, `Shell\Shell.lua`, `Modules\Raidframes.lua`, `Modules\ClickCast.lua`, `Modules\Share.lua`, `Options.lua`, `GameMenu.lua`. |
...
| `Shell/Tokens.lua` | **Suite-Shell Design-Tokens** (`ns.UI`): Farben `C` (Ink/Gold/Parchment-Rampen via `hex()`), Gold-Deckkräfte `line`, Schriften `FONT`/Rollen `ROLE` (+ `UI:SetFont`), Spacing `S`/Radien `R`, Panel-Maße `PANEL` (scale 0.74). **Zentrale Bau-Primitive** `UI.Border/Fill/FS/SetColor/GradientLine` (Shell + Widgets teilen sie). **`UI.WIDGET`** = ALLE Widget-Maße zentral (Höhen/Abstände/Box-Größen) — der eine Ort, an dem Florian feinjustiert. §10.11. |
| `Shell/Widgets.lua` | **Widget-Toolkit (Phase 2, `ns.W`):** wiederverwendbare Builder auf den Tokens — `SectionDivider, Field, Slider` (pointer-getrieben + editierbare Wert-Box), `Select` (Popover-Dropdown, Gold-Balken markiert die Auswahl), `Checkbox` (Blizzard-Check-Textur), `Button` (primary/ghost/danger, Schnitt je Variante), `Card, GroupPanel, IconTile, Row`. Maße aus `UI.WIDGET`, keine Magic Numbers. §10.11. |
| `Shell/Shell.lua` | **Suite-Shell-Chrome (`ns.Shell`):** Singleton-Panel (Header/Wordmark, Nav-Rail, Tab-Strip, Footer, Rune-Ecken via `CreateLine`, Close-X). `Build/Toggle/Show/Hide`, `SelectSection/SelectTab`. `RenderDummy` = live bedienbare Widget-Galerie (Sandbox-Daten, Phase 2). Aufruf `/lumen shell` (parallel zur AceConfig). §10.11. |
...
**Stand:** **v0.9.30**. Render, Dispel, Layout/Ausrichtung, Text/Outline, Base/Raid/Group-Tabs, Click-Cast (v0.9.6/.7), Export/Import (v0.9.8) und Aura-Indikatoren Phase 1 (v0.9.9) — alles live bestätigt. **Aura Phase 2 (v0.9.11):** 3 Kategorien, B2/B3-Whitelists (alle Klassen/Specs + Signatur-Lernen `db.global.auraSigs`), secret-Icons, Blizzard-Debuffs. **B4 Whitelist-Editor (v0.9.12–.19, live bestätigt):** eigener `Tracking`-Tab. Editor nur HoTs+Def (Debuffs über Filtermodi), je Kategorie Liste (Icon+Name+Entfernen) + Suche/Dropdown/Hinzufügen + „Standard wiederherstellen". Spell-Quelle `CC:GetAuraSpells()` = castbares Zauberbuch + **nur gewählte Talente** (`C_Traits`, declutter → Talent-Auren wie „Verschmelzung" wählbar). Getrackte aus Dropdown ausgeblendet. **Immer aktive Spec** (kein Switcher — Talente nur aktiv lesbar; Defaults decken andere Specs). Custom AceGUI-Item `LumenSpellDropdownItem` → **Spell-Tooltip im Dropdown** (Strata TOOLTIP + Frame-Level übers Pullout). **Render-Fix:** `hotsOwn`-Filter von `HELPFUL|PLAYER|RAID` → `HELPFUL` + `ownOnly`-Gate (`isFromPlayerOrPlayerPet`) → Proc-/Talent-HoTs (kein PLAYER-Flag) erscheinen jetzt; `wlType` matcht zusätzlich über `GetBaseSpell` + `PRIMARY_BY_ALT`. **Reset-Fix:** schreibt Defaults direkt zurück. Volldetails: §10.8 + Memory `lumen-aura-phase2-progress`. **v0.9.20–.25 — Bugfixes/Folge-Fixes (alle live bestätigt):** v0.9.20 wollte HP-Prozent („1%" bei vollem Leben) + HoT-Buff-Leak fixen, **riss aber alle Auren mit**: das `*100` lag AUSSERHALB des `pcall`, und ohne `ScaleTo100`-Kurve wirft `UnitHealthPercent` in 12.0.7 → `RenderLive` brach vor `RenderAurasLive` ab (Def-CDs verschwanden, obwohl deren Code unberührt = der Beweis). **v0.9.21-Fix:** `UnitHealthPercent(u, true, CurveConstants.ScaleTo100)` → garantiert non-secret 0..100, keine Arithmetik außerhalb pcall. Der HoT-no-fallback-Teil blieb (kuratierte Whitelist). **v0.9.22 Talent→Aura-Remap:** das Tracking-Dropdown bietet Talent-IDs (`C_Traits`); Proc-/Dupe-Talente legen aber eine Buff-Aura mit ANDERER ID an (Germination: Talent 155675 → Aura 155777). `TALENT_TO_AURA` + `Raidframes:ResolveTrackId` mappen beim Hinzufügen; `/ldump`-Debugkommando (Core.lua, eigene+Ziel-Buffs dumpen) findet weitere — **vor Release entfernen**. **v0.9.23/.25 Click-Cast sichere Defaults:** Links=Ziel, Rechts=Menü bleiben, bis der Nutzer BUTTON1/2 selbst belegt (kein Aussperren). **v0.9.24 Heiler-IDs:** alle 7 Heiler-Specs gegen EllesmereUI geprüft (alles echte Aura-IDs, keine Talent-IDs); einzige Lücke Dawnlight 431381 (Holy Pala) ergänzt. **v0.9.28 Aggro (§10.9) + v0.9.30 Sortierung (§10.10):** beide live bestätigt → MVP-Block komplett. **Offen:** Debuff-Live-Test (Florian, braucht Gruppe).
...
* **In-Kampf-Disziplin:** `secureLayoutDirty`-Flag + `PLAYER_REGEN_ENABLED`-Flush; Header/Buttons werden im Kampf nicht umgebaut. **Sortierung (v0.9.30)** über Header-Attribute `groupBy`/`groupingOrder`/`sortMethod` (Gruppe = INDEX, Rolle = ASSIGNEDROLE) — §10.10.
...
* ~~MVP-Reste: Aggro-Warnung · Sortierung Rolle/Gruppe~~ **✓ (v0.9.28/.30, live bestätigt — §10.9/§10.10).**
* **Suite-Shell Phase 1+2 ✓ (v0.9.31–.36, live bestätigt):** Chrome + Tokens + Fonts + `ns.W`-Widget-Toolkit, alle Maße zentral in `UI.WIDGET`/`UI.ROLE` (§10.11, Memory `lumen-suite-shell-preview-plan`). **← HIER GEHT ES WEITER: Suite-Shell Phase 3** — erster echter Screen (Raidframes→Raid) ans `db.profile` verdrahtet (Widget get/set direkt aufs Profil + `ns.Raidframes:UpdateLayout`), danach restliche Screens + **angedockte Live-Vorschau** (reuse den Nicht-Secure-Preview-Pool, der Testmodus geht darin auf). Florian hat ein Click-Cast-Mockup als Zielbild. **Noch offen aus Aura-Phase 2:** Debuff-Live-Test (Florian, braucht Gruppe).
...
**Spätere Bausteine:** abgerundete Ecken (Toggle + Stärke, via Mask-Textur), volle native Edit-Mode-Registrierung, Heilabsorb-Überlaufkante wieder aktivieren, **Suite-Shell-Optik mit angedockter Live-Vorschau** (nächster großer Schritt), dann Modul 2 (Unit Frames).
...
**Nächster großer Schritt: Suite-Shell** (siehe §10.5/§8). MVP-Reste (Aggro §10.9, Sortierung §10.10) sind erledigt & live bestätigt. **Noch offen aus Phase 2:** Debuff-Live-Test (Florian, braucht Gruppe). B4 ist erledigt (live bestätigt).

### 10.9 Aggro-Warnung (Raidframes) — Ist-Zustand (v0.9.28, live bestätigt)

Event-getrieben über `UnitThreatSituation(unit)` (0–3, **NICHT secret** → Vergleich/Show im Kampf erlaubt) + `UNIT_THREAT_SITUATION_UPDATE` (in `UNIT_EVENTS`, geroutet via `unitToButton`). Kein Polling.
* **2-stufig:** Status 1–2 = „Aggro droht" (gelb), Status 3 = „hat Aggro" (rot).
* **Pro Stufe** (`aggroModeAggro`/`aggroModeWarn`): `border` (Nur Rand) | `overlay` (Rand + Füllung); **„Aggro"-Text** nur im Overlay-Modus + eigener Toggle (`aggroTextAggro`/`aggroTextWarn`). Defaults: Rot = overlay+Text, Gelb = nur Rand.
* **Geteilte Optik** (Rot/Gelb nie gleichzeitig auf einem Frame): `aggroFillAlpha` + Text `aggroTextSize/Point/X/Y/Outline`. Farben `aggroColorAggro`/`aggroColorWarn`.
* **Tanks ausgenommen** (`UnitGroupRolesAssigned=="TANK"`) — sollen Aggro haben.
* **Eigene Schicht `f.aggroLayer`** (`base+10`, ÜBER den Aura-Holdern) trägt Füllung+Rand+Text → liegt über den Aura-Icons. `Raidframes:SetAggro(f, status)`, aufgerufen in `RenderLive`/`RenderFake`. Testmodus: Owlday rot, Tormund gelb.
* **Options** im `Base`-Tab als 3 Blöcke (Hat Aggro / Aggro droht / geteilte Darstellung). **UI-Konvention hier etabliert (suiteweit):** wertabhängige Unteroptionen **ausgrauen + `desc`-Tooltip** (nennt, was sie freischaltet); strukturell/kontextuelle ausblenden.

### 10.10 Sortierung nach Rolle/Gruppe (Raidframes) — Ist-Zustand (v0.9.30, live bestätigt)

Secure-konform über `SecureGroupHeader`-Attribute (EllesmereUI-Muster), in `LayoutLive` gesetzt (OOC; im Kampf via `secureLayoutDirty` aufgeschoben).
* **`sortMode`** (global, Base-Tab): `group` (Default — kein groupBy, `sortMethod=INDEX`) | `role` (`groupBy=ASSIGNEDROLE`, `sortMethod=NAME`, `groupingOrder = sortRoleOrder + ",NONE"`).
* **`sortRoleOrder`** = frei umsortierbare Prioritätsliste `{TANK,HEALER,DAMAGER}` (Options: 3 Inline-Boxen mit ▲/▼; global für Raid+Party).
* **`sortApplyRaid`** (Default aus): Rolle gilt im **Dungeon/Party immer**, im **Raid nur** wenn aktiviert (man baut Raids oft selbst nach Gruppe). `isRaidContext()` trennt das.
* Bei Sortier-Änderung **Hide→Attribute→Show** (sonst rechnet der Header weiter) — gebündelt mit dem bestehenden sizeChanged-Hide/Show.
* **Testmodus-Vorschau:** Fakes tragen `role`; `GetFakeList` sortiert stabil per Bucket (respektiert `sortApplyRaid`). Live erledigt der Header.
* **Akzeptierte Blizzard-Grenze:** Rolle braucht *zugewiesene* Rollen (alle echten Gruppen-Inhalte haben die). Offene Welt: fremde Spieler-Spec/Rolle nicht zuverlässig synchron lesbar (Inspect nötig) — EllesmereUI bestätigt dieselbe Grenze. Bewusst so akzeptiert.

### 10.11 Suite-Shell (eigene Config-Optik, `Shell/`) — Ist-Zustand (v0.9.36)

Eigene gerunte Einstellungs-Oberfläche nach dem **Lumen Design System**. Läuft **PARALLEL** zur bestehenden AceConfig: `/lumen shell` öffnet die Shell, `/lumen` bleibt die klassische AceConfig (bewusst gewählter Rollout — nichts wird abgeschaltet, bis die Shell die Inhalte wirklich trägt). Designquelle (außerhalb des Repos): Claude-Design-Prototyp unter `E:\Github\WoW Addon Einstellungsseite` (`tokens/*.css`, `components/core/*.jsx`, `ui_kits/lumen-config/*`). Volle Detail-Historie + technische Lessons: Memory `lumen-suite-shell-preview-plan`.

**Phase 1 (v0.9.31, gemergt) — Chrome + Tokens + Fonts:**
* **`Shell/Tokens.lua` (`ns.UI`):** Design-System → Lua. Farb-Rampen `C` (Ink/Gold/Parchment), Gold-Deckkräfte `line` (strukturelle Trenner bei 0.28 — 0.12 ist im Spiel unsichtbar), Schriften `FONT` + Rollen `ROLE`, `UI:SetFont`, `UI.Track` (Letter-Spacing-Emulation), Spacing `S`/Radien `R`, `PANEL` (1500×1000 @ scale 0.74).
* **`Shell/Shell.lua` (`ns.Shell`):** Singleton-Panel — Header/Wordmark, Nav-Rail, per-Sektion Tab-Strip, Footer, Vektor-Rune-Ecken (`CreateLine`), Close-X. `Build/Toggle/Show/Hide`, `SelectSection/SelectTab`.
* **`Fonts/`:** Cinzel SemiBold/Bold (Headings/Wordmark) + Hanken Grotesk Regular/Medium/SemiBold/**Bold** (Body/Controls), statische TTF aus Fontsource (latin inkl. Umlaute), SIL OFL + Lizenzdateien.
* **Technische Lessons (NICHT neu lernen):** **`PixelUtil.SetPoint/Width/Height` für ALLE 1px-Hairlines** (unter `SetScale(0.74)` verschwinden naive 1px-Linien) — der `UI.Border`-Helfer macht das. Border über **Eck-Anker** (TOPLEFT+BOTTOMLEFT), nicht TOP/BOTTOM+LEFT. Aktiv-Gradient auf ARTWORK, Borders auf OVERLAY (sonst überdeckt). Nav-Divider auf das rechte Geschwister-Frame (Child-Texturen werden von Child-Buttons überdeckt). `SetGradient` nimmt in 12.0 nur "HORIZONTAL"/"VERTICAL" + CreateColor.

**Phase 2 (v0.9.32–.36, live bestätigt im Screenshot-Loop) — Widget-Toolkit `ns.W` (`Shell/Widgets.lua`):**
* Wiederverwendbare Builder, Options-Tabellen-API (`o.get()`/`o.set(v)`), jedes Widget **selbst-dimensioniert**: `SectionDivider`, `Field` (Label über Control), `Slider` (pointer-getriebener Track + Knopf + **editierbare Wert-Box**: tippen → Enter, Live-Clamp auf Max), `Select` (Popover-Dropdown; **Gold-Balken links markiert die Auswahl**, Hover = warmer Braun-Wash → Selected ≠ Hover; Menü an das Widget geparentet → erbt Scale + räumt sich beim Tab-Wechsel auf), `Checkbox` (Blizzard-Check-Textur, entsättigt + ink-getönt), `Button` (primary/ghost/danger, **Schnitt je Variante**: primary Bold), `Card`, `GroupPanel` (Titel + optionales Header-Right-Control), `IconTile` (Gold-Chip), `Row` (N gleichbreite Zellen, row3/row2).
* **ZENTRALISIERUNG (wichtig — Florians ausdrücklicher Wunsch, beibehalten):** ALLE Widget-**Maße** in `UI.WIDGET` (Tokens.lua), ALLE **Schriftgrößen** in `UI.ROLE`. Widgets + Galerie lesen NUR daraus — **keine Magic Numbers im Widget-/Screen-Code**. Eine Token-Änderung schlägt überall durch; Florian justiert dort selbst (4er-Raster 12/16/20).
* **`Shell:RenderDummy`** = live bedienbare **Widget-Galerie** (Sandbox-Daten, noch NICHT db-verdrahtet) — das Screenshot-Stück, gleicher Inhalt auf allen Tabs (in Phase 2 Absicht).
* Vorbild-Pattern: EllesmereUIs `WidgetFactory` in `EllesmereUI_Widgets.lua` (Benchmark, nichts kopiert).

**Phase 3 (nächster Schritt) — echte Screens:** erster Screen Raidframes→Raid 1:1 nach Prototyp (`RaidScreen.jsx`), Widgets get/set direkt aufs `db.profile.raidframes` + `ns.Raidframes:UpdateLayout`/`RefreshAll`. Danach restliche Screens, dann die **angedockte Live-Vorschau** (Nicht-Secure-Preview-Pool oben einklinken; der eigenständige Testmodus-Schalter geht darin auf). Noch zu bauende Widgets: `KeybindButton`, `TextInput`, `Textarea` (für Click-Cast/Profile-Screens).
