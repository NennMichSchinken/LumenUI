...
Lade-Reihenfolge laut `Lumen.toc`: `embeds.xml` → `Core.lua` → `EditMode.lua` → `Style.lua` → `Shell\Tokens.lua` → `Shell\Widgets.lua` → `Shell\Screens.lua` → `Shell\Shell.lua` → `Modules\Raidframes.lua` → `Modules\ClickCast.lua` → `Modules\Share.lua` → `Options.lua` → `GameMenu.lua`.
