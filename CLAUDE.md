...
> Stand: Addon-Version **0.9.30** (**MVP-Raidframes-Block KOMPLETT** — alle §7-Features umgesetzt & live bestätigt). Seit v0.9.20: **v0.9.21–.25 Folge-Fixes** nach der v0.9.20-Regression (HP-%/Auren-Crash behoben via `UnitHealthPercent`+`ScaleTo100`-Kurve; **Talent→Aura-Remap** fürs Tracking — `TALENT_TO_AURA`/`ResolveTrackId`, Germination 155675→155777; Click-Cast **sichere Defaults** Links=Ziel/Rechts=Menü bis selbst belegt; **Heiler-IDs** auf Ellesmere-Standard inkl. Dawnlight 431381). **v0.9.28 Aggro-Warnung** (§10.9) + **v0.9.30 Sortierung Rolle/Gruppe** (§10.10), beide live bestätigt. **Offen: Debuff-Live-Test** (Florian, braucht Gruppe). **Temp `/ldump`-Debugkommando in `Core.lua` — vor Release entfernen.** **Nächster großer Schritt: Suite-Shell** (eigene gerunte Optik mit angedockter Live-Vorschau, Click-Cast-Mockup als Zielbild; Testmodus geht darin auf — Memory `lumen-suite-shell-preview-plan`). Interface **120007** (Retail-Patch 12.0.7, live seit 16.06.2026). Tiefe Details: Memories `lumen-aura-phase2-progress`, `lumen-suite-shell-preview-plan`.
...
|---|---|
| `Lumen.toc` | Deklariert Addon. `## Interface: 120007`, `## SavedVariables: LumenDB`, `## Author: NennMichSchinken`, `## Version: 0.9.30`. Lädt in Reihenfolge: `embeds.xml`, `Core.lua`, `EditMode.lua`, `Style.lua`, `Modules\Raidframes.lua`, `Modules\ClickCast.lua`, `Modules\Share.lua`, `Options.lua`, `GameMenu.lua`. |
...
* **← HIER GEHT ES WEITER: Suite-Shell** (eigene gerunte Optik mit angedockter Live-Vorschau; reuse den Nicht-Secure-Preview-Pool, der Testmodus geht darin auf — Memory `lumen-suite-shell-preview-plan`). Florian hat ein Click-Cast-Mockup als Zielbild. **Noch offen aus Phase 2:** Debuff-Live-Test (Florian, braucht Gruppe).
